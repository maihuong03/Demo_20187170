...
def episode():
    ...
    print("Action:", action)
    print("Q value: ", Q[current_state, action])
    if action == 0:
        current_pos[0] -= 1
    elif action == 1:
        current_pos[0] += 1
        ...
        current_pos[1] += 1
    print("Reward: ", reward[current_pos[0], current_pos[1]])
    print("Current Position: ({}, {})".format(current_pos[0], current_pos[1]))
    if reward[current_pos[0], current_pos[1]] == 5:
        print(shortest_path)
        exit()
...
root_path = "./map_stc"
env_simulation_path = os.path.join(root_path, "test_2.png")
# img = cv2.imread(os.path.join(dir_path, 'map', 'test_2.png'), cv2.IMREAD_GRAYSCALE)
img = cv2.imread(env_simulation_path, cv2.IMREAD_GRAYSCALE)
...
settings(route)
while True:
    print("HighestReward_Counter:", highestReward_counter)
    if highestReward_counter < repeatLimit:
        episode()
    else:
        # draw shortest path
